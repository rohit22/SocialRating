 ...
 *
 * 1. Retrieving the top-level comments for a video via "commentThreads.list"
 * method. 2. Replying to a comment thread via "comments.insert" method. 3.
 * Retrieving comment replies via "comments.list" method. 4. Updating an
 * existing comment via "comments.update" method. 5. Sets moderation status of
 * an existing comment via "comments.setModerationStatus" method. 6. Marking a
 * comment as spam via "comments.markAsSpam" method. 7. Deleting an existing
 * comment via "comments.delete" method.
 *
...
public class Comments {
	...
	public static void main(String[] args) {
		...
		try {
			...
			try {
				InputStream in = Comments.class.getResourceAsStream("/" + PROPERTIES_FILENAME);
				properties.load(in);
			...
			String videoId = "vOzNB-kN668";
			System.out.println("You chose " + videoId + " to subscribe.");
			printComments(videoId, properties.getProperty("youtube.apikey"));
			/*
			 * // Prompt the user for the comment text. // Retrieve the text
			 * that the user is commenting. String text = getText();
			 * System.out.println("You chose " + text + " to subscribe.");
			 * 
			 * // All the available methods are used in sequence just for the
			 * sake // of an example.
			 * 
			 * // Call the YouTube Data API's commentThreads.list method to //
			 * retrieve video comment threads. CommentThreadListResponse
			 * videoCommentsListResponse =
			 * youtube.commentThreads().list("snippet")
			 * .setVideoId(videoId).setTextFormat("plainText").execute();
			 * List<CommentThread> videoComments =
			 * videoCommentsListResponse.getItems();
			 * 
			 * if (videoComments.isEmpty()) { System.out.println(
			 * "Can't get video comments."); } else { // Print information from
			 ...
			 * 
			 * // Will use this thread as parent to new reply. String parentId =
			 * firstComment.getId();
			 * 
			 * // Create a comment snippet with text. CommentSnippet
			 ...
			 * youtube.comments().list("snippet").setParentId(parentId)
			 * .setTextFormat("plainText").execute(); List<Comment> comments =
			 * commentsListResponse.getItems();
			 * 
			 ...
			 * Comment commentUpdateResponse =
			 * youtube.comments().update("snippet",
			 * firstCommentReply).execute(); // Print information from the API
			 ...
			 * youtube.comments().setModerationStatus(firstCommentReply.getId(),
			 ...
			 * System.out.println("  -  Deleted comment as spam: " +
			 * firstCommentReply.getId()); } }
			 */
	...
	private static void printComments(String videoId, String apiKey) throws IOException {
		// Call the YouTube Data API's commentThreads.list method to
